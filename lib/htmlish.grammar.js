(function() {
  'use strict';
  var $parse, CND, GRAMMAR, Htmlish_grammar, MAIN, alert, assign, badge, dd, debug, echo, freeze, grammar, help, info, isa, jr, lets, log, new_datom, new_grammar, rpr, type_of, types, urge, validate, warn, whisper;
...
/* NOTE we assume that unique kids exist and that values are stored in source order */
    ...
    switch (/* may happen when parsing errors occur */token_name) {
      ...
      case 'osntag':
        ...
        if ((d.name != null) && d.name !== '') {
          e = this._parse_compact_tagname(d.name, true);
          if (e.id != null) {
            if (d.id != null) {
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._parse_compact_tagname = function(compact_tagname, strict = false) {
    var R, groups, k, pattern, ref, target, v, y;
  ...
  new_grammar = function(settings) {
    return GRAMMAR.new_grammar('Htmlish', MAIN, settings);
  };
